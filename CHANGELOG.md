...
- **Fix include-glob directory pruning** — sibling directories were incorrectly traversed when include patterns contained intermediate ancestors; e.g. `Users/Administrators/**` would also descend into `Users/Administrator` because `Users/` matched as an ancestor prefix; the fix distinguishes ancestor dirs (exact membership) from terminal dirs (descendant check), so only `Users/Administrators` and its children are visited
