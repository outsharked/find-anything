...
- **Rust dependency housekeeping (low/moderate risk)** — bumped `image` (0.25.6 → 0.25.9), `tiff` (0.10 → 0.11), `tower-http` (0.6 → 0.7), `dicom-object`/`dicom-dictionary-std`/`dicom-pixeldata` (0.9 → 0.10), `sevenz-rust2` (0.20 → 0.21), and `winreg` (0.52 → 0.56). No code changes needed; verified with `cargo build`/`clippy`/`test` across the workspace plus a cross-compile type-check against `x86_64-pc-windows-gnu` for the Windows-only crates.
  - `symphonia` (0.5 → 0.6) was attempted but reverted: the 0.6 probe/codec API needed a real rewrite in `find-extract-media`, and even after porting it, a test caught a correctness regression — the new ID3v2 reader silently drops `TCOM`/`TPE2` (composer/album-artist) frames from a real-world tagged MP3 that 0.5 read fine. Deferred alongside the other higher-risk extractor-library bumps (`nom-exif`, `scraper`, `calamine`, `quick-xml`, `kamadak-exif`, `gray_matter`, `rand`, `windows-sys`, `tray-icon`, `zip` dev-dependency) pending root-cause investigation.
