...
- **Testing gaps plan (plan 071) — phase 1 complete** — analysis of all test coverage gaps across Rust, TypeScript, and CI; phase 1 implemented: vitest suite added to CI web job (`pnpm test --run`); 17 new Rust unit tests covering `ArchiveManager` write operations (`append_chunks`/`remove_chunks`/`rewrite_archive` round-trips, stale-entry idempotency, subfolder path correctness), `process_file_phase1` (new/modified/skipped outcomes, content-hash dedup alias registration, inline vs. deferred storage selection), and `fts_candidates`/`fts_count` SQL queries (basic match, date filter, kind filter, `filename_only` mode, limit, count consistency)
