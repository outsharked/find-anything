...
- **`find-preview-dicom` was missing from release tarballs** — same class of bug as [#70](https://github.com/outsharked/find-anything/pull/70) (which added `find-extract-dicom`/`find-extract-pe` but missed this one): the release packaging `BINARIES` list in `.github/workflows/release.yml` never included `find-preview-dicom`, so the DICOM inline image viewer (`GET /api/v1/view` for `.dcm` files) failed with `failed to spawn find-preview-dicom: No such file or directory` in every release build, even though DICOM metadata extraction worked fine. ([#73](https://github.com/outsharked/find-anything/pull/73))
- **Image viewer "Fit to viewport" did nothing for images smaller than the panel** — the fit-scale calculation capped at 1 (native size) whenever the image already fit within the container, instead of scaling up to fill it. Now: opening an image defaults to native size unless it overflows the panel (in which case it auto-shrinks to fit), and the toolbar button is a real toggle between native size and fill-viewport (upscaling small images on request), with the icon highlighting to show which state is active. ([#72](https://github.com/outsharked/find-anything/pull/72))
...
- **`find-watch`/`find-scan`: changes to `.index`/`.noindex` control files now trigger a rescan (closes [#4](https://github.com/outsharked/find-anything/issues/4))** — previously, creating, editing, or deleting a `.index`/`.noindex` file was treated like any other filesystem change: `find-watch` would index the control file's own content as if it were regular text (or silently drop it on delete), and a manual `find-scan <path-to-control-file>` did the same — neither ever re-applied the new include/exclude rule to sibling files. Both now detect control-file events and rescan the affected directory instead. The rescan target is the control file's *grandparent* (one level above its own directory), not its immediate parent: the walker only evaluates `.noindex`-presence and `.index`-overrides for non-root walk entries, so scanning the control file's own directory as the walk root would never re-apply its own control file to itself (`resolve_scan_target` in `path_util.rs`, unit tested). `find-watch` spawns a scoped `find-scan <dir>` subprocess (`spawn_find_scan`, factored out of the existing periodic-scan spawn) via an injectable `rescan_parent` closure, mirroring the existing `register_dir` test pattern. Verified end-to-end against a live server and the real `find-scan` binary (`crates/server/tests/scan_control_files.rs`): adding `.noindex` to a subdirectory removes its files from the index, removing it restores them, and a newly added `.index` include filter is correctly applied.
- **Directory tree: request flood from live index events during a bulk scan** — `TreeRow.svelte` refreshed an expanded directory's listing on every matching live SSE index event, with no debounce or in-flight guard, and matched by `startsWith` against any descendant path (not just direct children). A bulk scan can emit one event per file, so a shallow expanded ancestor (e.g. a broad source root) fired one `listDir` request per file — hundreds of concurrent, unthrottled requests during a large scan. Replaced with a trailing-edge throttle (`treeRowLogic.ts`, unit tested): refreshes at most once per `liveRefreshIntervalMs` (new optional prop on `TreeRow`/`DirectoryTree`, default 1000ms) instead of on every event, so the row still updates promptly during a long burst rather than going silent until it fully stops (a plain debounce would have done that) while no longer flooding the network tab.
- **`crates/client/src/path_util.rs`: clippy `question_mark` CI failure** — `include_dir_prefixes` used a nested `match before.rfind('/') { None => return None, Some(slash) => &pat[..slash] }`, which newer clippy flags as replaceable with `?` (denied via `-D warnings` in CI). Simplified to `let slash = before.rfind('/')?; &pat[..slash]` — same behavior, no lint. Verified with `cargo clippy --workspace -- -D warnings` (with the `web/build` stub CI creates).
...
- **Inbox worker: coalesce consecutive requests into shared SQLite transactions (plan 093, closes [#59](https://github.com/outsharked/find-anything/issues/59))** — the follow-up lever to the per-request commit batching below: a burst of many separate single-file requests (e.g. a file watcher uploading one changed file at a time) previously still paid one full commit — WAL frames plus interior btree pages of a potentially multi-GB source DB — per request. The router now dispatches inbox `.gz` files in bounded groups (≤32 files / ≤8 MiB compressed), and consecutive same-source requests share one connection with a transaction committed every 25 write units *across* request boundaries (`SourceSession` in `worker/group.rs`), collapsing ~25 burst commits into one. Groups only contain files already queued at dispatch time, so nothing waits on future arrivals and quiet-period behavior is unchanged (group of one). Crash-safety invariant: an inbox `.gz` is deleted only at a flush point — after the COMMIT covering all its writes and after its normalized to-archive `.gz` is on disk (buffered in memory until then, because phase 2's stale-hash check reads `files.file_hash` and must never see a payload whose hash isn't committed yet). A crash mid-group leaves unflushed files in `inbox/` to be reprocessed idempotently, same recovery unit as before. The `inbox_request_timeout_secs` timeout now bounds a group; on timeout the current connection is interrupted via a shared `InterruptHandle` slot and only the in-flight request is quarantined to `failed/`. The four DB write helpers (`delete_files_phase1`, `rename_files`, `log_activity`, `do_cleanup_writes`) now detect an open transaction (`is_autocommit`) instead of unconditionally opening their own. Covered by new group-level unit tests (cadence crossing, mixed-source session switching, corrupt-gz quarantine, cross-request delete ordering) and a 40-request burst integration test (`burst_coalescing.rs`).
...
- **File viewer: virtualized scrolling for large files (plan 092)** — paged mode previously accumulated forever: every page ever scrolled through stayed mounted as real DOM rows, so a long scroll session left thousands of permanent `<tr>`s behind. The viewer now keeps a bounded window (one page size, default 2000 rows) positioned inside the scroll container by spacer rows, so the scrollbar reflects the whole file from the moment it opens and the DOM stays constant no matter how far you scroll. Scroll-driven window fetches are debounced (100 ms) and superseding — a fast scrollbar drag issues a couple of requests, not dozens — and the old window dims (matching the search-results stale treatment) while its replacement loads. Opening a file at a line (search result, `#L` deep link) positions directly via the window math instead of the old anchor-page + `scrollIntoView` dance.
  - **Two rendering paths, two components.** `FileViewer.svelte` is now a shell (metadata, toolbar, media/formatted viewers) that mounts exactly one text view: `WindowedCodeView.svelte` (new, virtualized; used when paged and word-wrap is off) or `PagedCodeView.svelte` (the legacy accumulate path extracted verbatim; used for word-wrap mode and small files, since variable-height rows can't be virtualized without measure-and-correct machinery). Toggling word-wrap swaps components and preserves the current reading position. Shared building blocks: `CodeViewer` (grew optional spacer-row props), a `fetchLineRange()`/`normalizePage()` helper (`fileContent.ts`), and pure windowing math in `virtualWindow.ts` (unit-tested; all geometry in raw-line space, with gap-filling placeholder rows for server-skipped blank lines so spacer heights never drift).
  - Known trade-offs (virtualized path only): browser find-in-page and text selection only cover the rendered window, matching how virtualized editors (VS Code/Monaco) behave; formatted markdown view of a paged file renders from the initially-loaded page rather than growing as you scroll.
  - Integrates with the Ctrl+G / hash-edit jumps added in [#62](https://github.com/outsharked/find-anything/pull/62): both view components export `jumpToLine()`, and the shell's selection-change effect delegates to whichever is mounted — the windowed path fetches a centered window and positions directly; the legacy path keeps the scroll-or-reanchor behavior.
...
- **Rust dependency: `nom-exif` 2.8 → 3.6 in `find-extract-media`** — before bumping, discovered that every existing video test only covered the magic-byte fallback path (AVI/FLV/MPEG/OGG/WMV); none exercised `extract_video_nom_exif`, the code that actually calls into `nom-exif` for MP4/MOV/MKV/WebM. Added a real MP4 fixture (`testdata/tiny.mp4`, generated with `ffmpeg`) and a test asserting resolution/duration extraction, verified against 2.8 first as a baseline. The 3.x API changes: `MediaSource::file_path()` → `MediaSource::open()`; `MediaParser::parse()` → `MediaParser::parse_track()` for video/audio track info; `TrackInfoTag::ImageWidth`/`ImageHeight` → `Width`/`Height`; and `MediaSource::has_track()` was removed since `parse_track()` now returns `Error::TrackNotFound` directly for track-less sources (handled by the existing error fallback, no behavior change).
...
- **Svelte runes migration (incremental, 6/6 — final)** — `FileViewer`, `+page.svelte`, `+layout.svelte`, `settings/+page.svelte`, `v/[code]/+page.svelte` converted from legacy syntax to runes, completing the migration started after the Svelte 5/Vite 8/TypeScript 6 toolchain bump. Highlights:
  ...
  - `+page.svelte`: found and fixed a real bug surfaced by the `fileView`/`selectedSources` state conversion — `history.pushState`/`replaceState` failed at runtime ("could not be cloned") because Svelte 5's `$state` wraps arrays in reactive proxies, which structured clone can't serialize. Fixed by unwrapping with `$state.snapshot()` in `captureState()` before the array fields reach `history.state`; `currentFile` (a `FilePath` class instance) is left untouched there since `serializeState()` already strips it to a plain path string for the same reason.
  - `v/[code]/+page.svelte`: a local variable literally named `state` collided with the `$state` rune (`$state('loading')` parsed as store auto-subscription to a variable called `state`), producing a `state` store error — renamed to `viewState`.
  - Verified live end-to-end: search → open into `FileViewer`, word-wrap toggle, share-dialog open/close, code-line selection, back navigation, settings page nav (desktop + accordion), markdown formatted-view toggle, sidebar resize (pointer and keyboard), and opening an image result.
...
- **Svelte runes migration (incremental, 4/N)** — `CommandPalette`, `DirListing`, `PathBar`, `TreeRow`, `SearchBox`, `DirectImageViewer` converted from legacy syntax to runes, plus follow-up touches to `DirectoryTree`/`MultiSourceTree` (Group 2/3) to forward the new `onOpen` callback now that `TreeRow` no longer dispatches a `CustomEvent`. Highlights:
  ...
  - `SearchBox`'s `query` prop is one-way from the caller but also mutated locally while typing (no `bind:query` anywhere) — ported as a local `$state` seeded from the prop plus an `$effect` that re-syncs only when the prop's own value changes, matching Svelte 4's per-variable dirty-checking semantics exactly. `isTyping`, by contrast, genuinely is two-way (`bind:isTyping` at the call site), so it became `$bindable`. Bare `on:focus`/`on:blur` DOM-event-forwarding shorthand (removed in Svelte 5) became explicit `onFocus`/`onBlur` callback props.
  - `DirectImageViewer` has no dispatch at all, just heavy imperative pan/zoom/rotate/filter state. Kept all explicit `applyTransform()` calls at their existing call sites (for immediate, non-batched visual feedback during drag) rather than consolidating everything into effects, to avoid risking perceptible drag lag; the one reactive block that has no explicit call site (flip toggling) became an `$effect`, verified live to still update the CSS transform correctly.

  Confirmed a small number of pre-existing dead/unused code paths along the way (an unused `onMount` import in `DirListing`, an unused `archivePathOf` import in `CommandPalette`) — left alone as out of scope for this migration.

### Changed

- **Svelte runes migration (incremental, 3/N)** — `CodeViewer`, `DirTypeahead`, `DirectHeader`, `ErrorsPanel`, `FileStatusBanner`, `MultiSourceTree`, `ResultList`, `SourceSelector` (unused/dead component) converted from legacy syntax to runes. This group's main pattern is `createEventDispatcher` → callback props (e.g. `onSelect`, `onNavigate`, `onOpen`), which required updating every call site (`TopBar`, `FileViewer`, `SearchView`, `+page.svelte`, `settings/+page.svelte`) alongside the component itself. `CodeViewer` and `SourceSelector` also had a prop that was both read and locally reassigned (Svelte 4's implicit "controlled prop" pattern) — since Svelte 5 doesn't allow mutating a non-`$bindable` prop, these now compute the next value and hand it to the caller via callback instead of mutating in place, with no `$bindable` needed since the caller was never using `bind:` in the first place. Components not yet migrated (`TreeRow`, `SearchResult`) still dispatch via `createEventDispatcher`, and their forwarding through migrated parents (`DirectoryTree`'s `on:open`, `ResultList`'s `on:open`) continues to work unmodified — Svelte 5 only flags `on:event` as deprecated for recognized native DOM event names, not custom component-dispatched ones.
...
- **Windows client.toml log dir** — `C:\Users\jamie\.config\FindAnything\logs` pre-configured in the Windows client config; `**/bin/Release/**` and `**/bin/Debug/**` added to `exclude_extra` to avoid indexing build artifacts.

### Fixed

- **Spurious mass reindex on Windows file creation** — directory `Modify` events (generated by Windows `ReadDirectoryChangesW` when any file is created inside a watched directory) no longer trigger `register_dir`, which previously caused a full subtree re-walk and a flood of spurious "update" events for every existing file in the directory. Only directory `Create` events now trigger watch registration.
- **SQLite page cache unbounded growth** — `PRAGMA cache_size = -16384` (16 MB) is now applied to every connection (both write and read-only) for `blobs.db` and source databases; previously uncapped caches could consume gigabytes under load.
- **Blobs.db read connection pool cap reduced** — `DEFAULT_MAX_READ_CONNECTIONS` dropped from 100 to 16, limiting the maximum number of simultaneous SQLite page caches open against `blobs.db`.
