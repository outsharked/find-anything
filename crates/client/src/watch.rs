...
fn build_source_map(sources: &[SourceConfig]) -> SourceMap {
    ...
    for src in sources {
        ...
        let terminals = if src.include.is_empty() {
            None
        } else {
            crate::path_util::include_dir_prefixes(&src.include)
        };
...
///
/// The walk and the watch registration are intentionally separated: collecting
/// all directories first (before any watches exist) avoids a flood of inotify
/// OPEN/ACCESS events generated by walkdir itself, which on older kernels can
/// overwhelm the notify background thread's read loop and cause the
/// `watcher.watch()` channel to deadlock.
fn watch_tree(
    ...
    // Phase 1: collect all directories without any watches active yet.
    let mut dirs: Vec<std::path::PathBuf> = Vec::new();
    crate::walk::walk_source_tree(root, root, scan, excludes, terminals, |item| {
        if let crate::walk::WalkItem::Dir(dir_path) = item {
            dirs.push(dir_path);
        }
    });
    info!("walk complete: {} directories found, registering watches", dirs.len());

    // Phase 2: register watches now that the walk is done.
        ...
        match watcher.watch(dir_path, RecursiveMode::NonRecursive) {
            Ok(()) => {
                ...
                if count.is_multiple_of(500) {
                    info!("registering watches... {} so far", count);
                }
            }
            Err(e) => {
