...
if [ "${SKIP_CONFIG:-0}" = "1" ]; then
  ...
  echo ""
  echo "  user   — installs as a systemd user service (RECOMMENDED for most users)"
  echo "           runs as your own user account"
  echo "           can read any file you have access to (home dir, personal files)"
  echo "           starts automatically when you log in"
  echo "           config: ~/.config/find-anything/server.toml"
  echo "           data:   ~/.local/share/find-anything"
  echo ""
  echo "  system — installs as a system service (for dedicated servers / multi-user)"
  echo "           requires root/sudo"
  echo "           runs as a dedicated 'find-anything' user with no home directory"
  echo "           WARNING: cannot read files in your home dir or owned by other users"
  echo "                    unless you manually grant access to the service account"
  echo "           config: /etc/find-anything/server.toml"
  echo "           data:   /var/lib/find-anything"
  echo ""
  printf "Install mode [user/system] (default: user): "
